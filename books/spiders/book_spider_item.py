...
class BookSpiderSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        data = d.findAll()
        for i in data:
            yield scrapy.Request(i[1],
                                meta={"proxy":BookSpiderSpider.__proxy},
                                cb_kwargs={"sku":i[0]},
                                callback=self.parse
                                )
       

    def parse(self, response,sku):
        ...
        try:
            ...
            text = requests.get(
                ...
                headers={"Referer":url},
                proxies={'http':BookSpiderSpider.__proxy,'https':BookSpiderSpider.__proxy},
                timeout=5
                ).text
            if(text.strip() != ""):
